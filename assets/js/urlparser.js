(function ($) {
	...
	/**
	 * Does the heavy lifting of changing all the special characters into url-valid text
	 *
	 * the string that will be urlified
	 * @param {string} str
	 *
	 * defines if the url will have as separator a "-" char or another one.
	 ...
	 * @constructor
	 */
	function FormatForUrl(str, separator) {

		// implementation to remove accents, diacritics, etc...
		...
		// remove and replace the rest of the special chars
		return str.replace(/_/g, separator.toString())
			.replace(/-/g, separator.toString())
			.replace(/ /g, separator.toString())
			.replace(/:/g, separator.toString())
			.replace(/\\/g, separator.toString())
			.replace(/\//g, separator.toString())
			.replace(/[^a-zA-Z0-9\-_]+/g, '')
			.replace(/-{2,}/g, separator.toString())
			.replace(/_{2,}/g, separator.toString())
			.toLowerCase();
	}

	var parserMethods = {
		// field that will hold the original string that will be urlified
		source: null,
		// field that will hold the urlified string
		parser: null,
		// button that changes the parser from editable to non editable
		button: null,
		// char that will define if the word separator of the urlified string it's an "-" or another one
		separator: null,
		...
		checkButton: function (clickEvent) {
			if ($(this.button).hasClass('btn-primary') == false) {
				...
				if (clickEvent == true) {
					$(this.parser).val(FormatForUrl($(this.source).val(), this.separator));
				}
		...
		activateParser: function () {

			$(this.parser).off('keyup');

			$(this.parser).prop('readonly', true);

			var object = this;

			$(this.source).on('keyup', function () {
				$(object.parser).val(FormatForUrl(this.value, object.separator));
			});
